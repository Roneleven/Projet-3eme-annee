...
public class GatlinLauncherPattern : MonoBehaviour
{
    ...
    private IEnumerator LaunchCubesOneByOne(List<GameObject> cubes)
    {
        ...
        int cubesLaunched = 0; // Variable pour compter les cubes lanc̩s

        foreach (GameObject cube in cubes)
        {
            if (cubesLaunched >= cubesToLaunch)
            {
                // Si le nombre de cubes lanc̩s atteint cubesToLaunch, sortir de la boucle
                yield break;
            }
            ...
            cubesLaunched++; // Incr̩menter le nombre de cubes lanc̩s
